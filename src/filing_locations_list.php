<?php
  ...
  $filing_location_data = get_all_filing_locations($filter, ['offset'=> $offset, 'total_records_per_page' => TOTAL_RECORDS_PER_PAGE]);
  
  $filing_locations = $filing_location_data['result'] ?? [];
  $total_records = $filing_location_data['total'] ?? 0;

  $pagy = pagination($total_records, $page_no); // setup pagination
  $modal_title = "Filing Location Details";
?>
        ...
        <?php include 'layouts/_navbar.php'; ?>
          ...
          <section class="content-header">
            <div class="container-fluid">
              <div class="row mb-2">
                <div class="col-sm-6">
                  <h1>Manage Filing Locations</h1>
                </div>
                <div class="col-sm-6">
                  <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item">Filing Location</li>
                  </ol>
                </div>
              </div>
            </div><!-- /.container-fluid -->
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="menu mt-5 mb-3">
                    <a href="/filing_locations/new" class="btn bg-gradient-success btn-md"><i class="fa-solid fa-plus"></i> New </a>
                  </div>
                  <div class="card">
                    <div class="card-header">
                      <h3 class="card-title">Filing Locations</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body p-0">
                      <div class="card-body table-responsive p-0">
                        <table class="table table-striped table-head-fixed text-nowrap">
                          ...
                          <tbody>
                            <?php if(!empty($filing_locations)) { ?>
                              <?php foreach($filing_locations as $key => $value ) { ?>
                                  <tr>
                                      <td><?= ++$key ?></td>
                                      <td><?= $value['name'] ?></td>
                                      ...
                                      <td class="action-buttons">
                                          <button data-url="/filing_locations/view/<?= $value['id'] ?>" class="btn bg-gradient-info btn-sm view-modal"><i class="fa-solid fa-eye"></i></button>
                                          <a href="/filing_locations/edit/<?= $value['id'] ?>" class="btn bg-gradient-primary btn-sm"><i class="fa-solid fa-pencil"></i></i></a>
                                          <a href="#" class="btn bg-gradient-danger btn-sm btn-delete" data-id="<?= $value['id'] ?>" data-url="/filing_locations/delete"><i class="fa-solid fa-trash"></i></a>
                                      </td>
                                  </tr>
                              <?php } ?>
                            <?php } else { ?>
                                <td colspan="9">No filing location(s) to display.</td>
                            <?php } ?>                
                          </tbody>
                        </table>
                      </div>
